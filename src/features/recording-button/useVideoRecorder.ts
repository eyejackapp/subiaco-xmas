...
export function useVideoRecorder(
  ...
  useEffect(() => {
    ...
    const handleStart = () => {
      if (state !== "none")
        console.warn(
          `Recording started but state not None. Instead found ${state}`
        );
      // console.log('Start');
      setState("recording");
    };
    const handleStop = () => {
      if (state !== "recording")
        console.warn(
          `Recording stopped but state not Recording. Instead found ${state}`
        );
      // console.log('Stop');
      setState("encoding");
    ...
    const handleReady = async (blob: RendererEvents["recording-ready"]) => {
      if (state !== "encoding")
        console.warn(
          `Recording ready but state not Processing. Instead found ${state}.`
        );
      setVideoBlob(blob);
    ...
    const handleRecordingProgress = (
      model: RendererEvents["recording-progress"]
    ) => {
  ...
  const handleStartRecording = useCallback(() => {
    if (!api)
      throw new Error("Cannot start recording until renderer api is provided.");
    api.startRecording();
  }, [api]);
    ...
    (filename: string) => {
      if (videoBlob) {
        try {
          ...
          navigator.share({
            title: "Subiaco Twilight Trail - Twinkling Treasure Hunt",
            text: "Immerse yourself in the Subiaco Twilight Trail and experience 8 lighting installations come to life with an augmented reality treasure hunt! Keep an eye out for special surprises and gifts along the way.",
            files: [videoFile],
          });
  ...
  if (state === "none") {
    ...
    if (!videoBlob)
      throw new Error(
        'useVideoRecorder: Something set state to "ready" but videoBlob is still undefined.'
      );
    if (!videoUrl)
      throw new Error(
        'useVideoRecorder: Something set state to "ready" but videoUrl is still undefined.'
      );
    return {
      state,
      videoUrl,
    ...
    throw new Error(
      `useVideoRecorder: Unhandled state type ${state}.  Please implement/fix.`
    );
  }
}
